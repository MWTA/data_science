...
def get_synsets(list_words, option):
    ...
    for word in list_words:
        try:
            ...
            if option == 1:
                item = { "category": word, "synset_category": wn.synset(word) }
            ...
            item = { "word": word, "synset_word": wn.synsets('other')[0] }
            list_synsets.append(item)
...
def save_prediction(data):
    with open(path_root+'out/classified_words.json', 'w') as file:
        for label in data:
...
if __name__ == '__main__':
    ...
    for word_synset in get_synsets(list_words, 0):
        ...
        prediction.append(selected['synset_category']['category'].split(".")[0])
